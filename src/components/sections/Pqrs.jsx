...
const Pqrs = () => {
  const [form, setForm] = useState({ name: "", email: "", type: "Petición", message: "" });
  const { sendEmail } = useEmailJS();
  const { showToast } = useToast();

  const handleSubmit = async (e) => {
    e.preventDefault();
    const result = await sendEmail(import.meta.env.VITE_EMAILJS_TEMPLATE_ID_PQRS, form);
    if (result.success) {
    ...
    <section id="pqrs" className="py-20 bg-white">
      <div className="container-custom max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="bg-sena-graybg p-8 rounded-2xl shadow-sm">
          ...
          <Button type="submit" variant="primary" className="w-full">Enviar PQRS</Button>
        </form>
      </div>
    </section>
  );
